...
outFile = [myObj.defaultDir 'movies/combo_movie' num2str(movNum)]

writerObj=VideoWriter(outFile);
writerObj.FrameRate=30;
open(writerObj);

oldmin = min(min(min(mov))); %-2^15;
oldmax = max(max(max(mov))); %2^15;
oldrange = oldmax - oldmin;
newmin = 0;
newmax = 255;
newrange = newmax - newmin;

% movie=((mov-min(mov(:)))/(max(mov(:))-min(mov(:))))*255;
% movie=((movie-min(movie(:)))/(max(movie(:))-min(movie(:))));

fArray = (((mov - oldmin) * newrange) / oldrange) + newmin;
[sizeY,sizeX,sizeZ]=size(fArray);


for f=1:sizeZ
    writeVideo(writerObj, double(fArray(:,:,f))/255);   
...
radiusPxCov = 12; %JYR 11; %default, may need zoom level adjustment
temporalBin = 1; % JYR %8; %default (tested w/ 15-30hz imaging), may need adjustment based on frame rate

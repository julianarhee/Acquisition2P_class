function SC2Pinit_noUI_crossref(obj, movPath, crossref)
...
if crossref
    uniquenames = {};
    for movidx = 1:length(movNames)
        acqNamePlaces = find(movNames{movidx} == '_');
        acqNamePlace = acqNamePlaces(end);
        uniquenames{end+1} = movNames{movidx}(1:acqNamePlace-1);
    end
    uniquenames = unique(uniquenames);
end
obj.acqSubNames = uniquenames;
...
try
    ...
    %instead
    pparts = strsplit(movPath, '/');
    obj.acqName = pparts{end};
